...
// src/index.js
var name = "kaori-preview";
var inject = ["connection", "sessions"];
var Config = Schema.object({
  maxReadBytes: Schema.natural().min(1).default(262144),
  maxEntries: Schema.natural().min(1).default(2e3),
  ignoreDirs: Schema.array(Schema.string()).default([".git", "node_modules", ".dsh", ".cache", ".claude", ".codex", ".cursor", ".vscode", "backup"]),
  imageExts: Schema.array(Schema.string()).default([".png", ".jpg", ".jpeg", ".gif", ".webp", ".svg", ".bmp"]),
  textExts: Schema.array(Schema.string()).default([".md", ".txt", ".json", ".js", ".mjs", ".cjs", ".ts", ".tsx", ".jsx", ".py", ".css", ".html", ".htm", ".xml", ".yaml", ".yml", ".toml", ".ini", ".sh", ".ps1", ".bat", ".cmd", ".sql", ".log", ".csv", ".tsv", ".env", ".gitignore", ".dockerignore", ".editorconfig", ".npmrc", ".gitattributes"]),
  // 面板打开时隐藏的 DOM 选择器（适配各种主题的装饰层，如侧边立绘/顶部台词）。
  // 默认主题无装饰，无需配置；其他主题按需添加，如：['.kaori-companions', '.kaori-top-decor']
  hideOnOpen: Schema.array(Schema.string()).default([])
});
var CHANNEL = "/kaori-preview";
...
function isImage(name2, cfg) {
  const ext = path.extname(name2).toLowerCase();
  return cfg.imageExts.includes(ext);
}
function isText(name2, cfg) {
  const ext = path.extname(name2).toLowerCase();
