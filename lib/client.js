...
function startUI(rpcCall, ctx, sessionInfo) {
  ...
  function createPanel() {
    ...
    panel.innerHTML = '<div class="kp-header">  <div class="kp-title-wrap">    <span class="kp-title">\u{1F4C2} \u6587\u4EF6\u9884\u89C8</span>    <span class="kp-path" id="kaori-preview-path" title=""></span>  </div>  <button class="kp-close" type="button" title="\u5173\u95ED">\u2715</button></div><div class="kp-body">  <div class="kp-tree" id="kaori-preview-tree"></div>  <div class="kp-view" id="kaori-preview-view">    <div class="kp-empty">\u9009\u62E9\u5DE6\u4FA7\u6587\u4EF6\u67E5\u770B\u9884\u89C8</div>  </div></div>';
  ...
  function refreshTree() {
    ...
    previewRpc("listDir", { path: "" }).then(function(res) {
      ...
      if (res.cwd) {
        ...
        if (pathEl) {
          pathEl.textContent = res.cwd;
          pathEl.title = res.cwd;
        }
      }
  ...
  function nodeHtml(dirPath, depth) {
    ...
    for (var i = 0; i < entries.length; i++) {
      ...
      if (e.isDir) {
        ...
        html += '<div class="kp-dir" style="padding-left:' + pad + 'px" data-path="' + escapeHtml(e.path) + '"><span class="kp-dir-arrow">' + (open ? "\u25BE" : "\u25B8") + '</span><span class="kp-dir-name">\u{1F4C1} ' + escapeHtml(e.name) + "</span></div>";
        if (open) {
        ...
        html += '<div class="kp-file" style="padding-left:' + pad + 'px" data-path="' + escapeHtml(e.path) + '"><span class="kp-file-icon">' + fileIcon(e.name) + '</span><span class="kp-file-name">' + escapeHtml(e.name) + "</span></div>";
      }
    }
    return html;
  }
  function renderTree() {
    ...
    if (currentRootPath) {
      ...
      rootRow = '<div class="kp-dir kp-root" style="padding-left:4px;font-weight:700"><span class="kp-dir-name">\u{1F4C1} ' + escapeHtml(rootName) + "</span></div>";
    }
